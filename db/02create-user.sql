...
(
  ...
  verify_string character varying(128) DEFAULT encode(gen_random_bytes(64), 'hex'::text), -- String needed to verify the user's email address.
  ...
  joined timestamp without time zone NOT NULL DEFAULT now(), -- When the user joined.
...
COMMENT ON COLUMN "user".joined IS 'When the user joined.';
